                ...
                link: function ($scope, element, attrs, controller) {
                    ...
                    function select() {
                        ...
                        if (prediction.is_custom) {
                            $scope.$apply(function () {
                                var model = prepareModelText(prediction.place);
                                $scope.model = model;
                                $scope.$emit('g-places-autocomplete:select', model);
                                $timeout(function () {
                            ...
                            placesService.getDetails({ placeId: prediction.place_id }, function (place, status) {
                                if (status == google.maps.places.PlacesServiceStatus.OK) {
                                    $scope.$apply(function () {
                                        ...
                                        var model = prepareModelText(place);
                                        $scope.model = model;
                                        $scope.$emit('g-places-autocomplete:select', model);
                                        $timeout(function () {
                                            controller.$viewChangeListeners.forEach(function (fn) { fn(); });
                    ...
                    function prepareModelText(place) {
                        var street_number = "";
                        var route = "";
                        var locality = '';
